...
void OnTxDone(void) {
  // Se há um STATUS agendado (após ACK de CMD), envia agora
  // antes de voltar ao RX — rádio ainda está acordado aqui
  if (pendingStatus) {
    pendingStatus = false;
    // sendStatus() chama Radio.Send() diretamente — permanece em TX
    // OnTxDone será chamado novamente após esse segundo TX
    char payload[32];
    ...
    return;   // sai sem mudar radioState — OnTxDone virá de novo
  }

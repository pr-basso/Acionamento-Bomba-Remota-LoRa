...
bool          pendingStatus = false;   // STATUS agendado — enviado após OnTxDone do ACK
...
void OnTxDone(void) {
  if (pendingStatus) {
    // Envia STATUS logo após o ACK ter saído completamente
    pendingStatus = false;
    char payload[32];
    ...
    Serial.printf("[TX] STATUS agendado: \"%s\"\n", pendingMsg);
    // OnTxDone será chamado novamente após STATUS sair
    return;
  }
  // TX concluído (ACK, STATUS, PONG, CMD...) — abre RX imediatamente
  // aqui, sem depender do próximo ciclo do loop()
  Radio.Rx(0);
  radioState = APP_STATE_LOWPOWER;
  Serial.println("[OnTxDone] TX ok — RX aberto");
}
...
void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr) {
  ...
  Radio.Sleep();
  Serial.printf("[OnRxDone] size=%d RSSI=%d SNR=%d raw=\"%s\"\n",
...
void OnRxError(void) {
  Radio.Sleep();
  Serial.println("[OnRxError] Erro CRC fisico ou preambulo — pacote descartado pelo radio");
...
bool parseMsg(const char *raw, char fields[][32]) {
  ...
  for (int i = 0; i < TOTAL_FIELDS; i++) {
    ...
    if (i < TOTAL_FIELDS - 1 && sep == nullptr) {
      Serial.printf("[PARSE] FALHA: campo %d sem separador '|'\n", i);
      return false;
    }
    if (sep) *sep = '\0';
  ...
  Serial.printf("[PARSE] SRC=%s DST=%s SEQ=%s TYPE=%s PAY=%s CRC=%s\n",
  ...
  if (expected != received) {
    Serial.printf("[PARSE] FALHA CRC: esperado=%02X recebido=%02X body=\"%s\"\n",
                  expected, received, body);
    return false;
  }
  if (strcmp(fields[FIELD_DST], MY_ADDRESS) != 0) {
    Serial.printf("[PARSE] DESCARTADO: DST='%s' != MY_ADDRESS='%s'\n",
                  fields[FIELD_DST], MY_ADDRESS);
    return false;
  }
...
void sendRaw(const char *msg) {
  Radio.Sleep();   // obrigatório: sai do RX antes de entrar em TX
...
void handleReceived() {
  ...
  if (strcmp(type, "CMD") == 0) {
    // 1) Envia ACK imediato — Radio.Send() dispara TX
    sendAck(seq);

    // 2) Executa a ação no relé
    ...
    // 3) Agenda STATUS para ser enviado em OnTxDone (após o ACK sair)
    //    NÃO chama Radio.Send() aqui — evita duplo TX simultâneo
    pendingStatus = true;
    return;
  }
}
...
void loop() {
  ...
  // ── Diagnóstico periódico a cada 5s ──────────────────
  static unsigned long lastDiag = 0;
  if (now - lastDiag >= 5000) {
    lastDiag = now;
    Serial.printf("[DIAG] radioState=%d waitingAck=%d retries=%d uptime=%lus\n",
                  (int)radioState, waitingAck, retries, now / 1000);
  }

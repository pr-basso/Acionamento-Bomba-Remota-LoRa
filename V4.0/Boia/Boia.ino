...
void OnTxDone(void) {
  // Abre RX imediatamente após TX concluído — sem esperar o loop()
  // Elimina a janela cega onde o nó remoto poderia responder e ser perdido
  Radio.Rx(0);
...
void OnRxTimeout(void) {
  Radio.Sleep();
  // Timeout é normal — apenas indica que não chegou pacote no período
  radioState = APP_STATE_RX_TIMEOUT;
}

void OnRxError(void) {
  Radio.Sleep();
  Serial.println("[OnRxError] Erro de recepção (CRC físico ou preâmbulo)");
...
bool parseMsg(const char *raw, char fields[][32]) {
  ...
  if (strcmp(fields[FIELD_DST], MY_ADDRESS) != 0) {
    Serial.printf("[PARSE] DESCARTADO: DST='%s' mas MY_ADDRESS='%s'\n",
